...
//    }
    ...
    public Set<String> getHoliday(String yyyy){
        holidaysSet.clear();
...
//        holidaysSet.add(yyyy+SolarDays(yyyy, "0816"));  // ""
        ...
        try {
            // 어린이날 대체공휴일 검사 : 어린이날,3.1절, 광복절, 개천절 및 한글날은 토요일, 일요일인 경우 그 다음 평일을 대체공유일로 지정
            ...
            int threeOneDayChk = LocalDate.parse(yyyy+"0301", formatter).getDayOfWeek().getValue();
            if(threeOneDayChk == LD_SUNDAY) {      // 일요일
                holidaysSet.add(yyyy+"0302");
            }
            if(threeOneDayChk == LD_SATURDAY) {  // 토요일
                holidaysSet.add(yyyy+"0303");
            }
            int kwangBokDayChk = LocalDate.parse(yyyy+"0815", formatter).getDayOfWeek().getValue();
            if(kwangBokDayChk == LD_SUNDAY) {      // 일요일
                holidaysSet.add(yyyy+"0816");
            }
            if(kwangBokDayChk == LD_SATURDAY) {  // 토요일
                holidaysSet.add(yyyy+"0817");
            }
            int gaeChunDayChk = LocalDate.parse(yyyy+"1003", formatter).getDayOfWeek().getValue();
            if(gaeChunDayChk == LD_SUNDAY) {      // 일요일
                holidaysSet.add(yyyy+"1004");
            }
            if(gaeChunDayChk == LD_SATURDAY) {  // 토요일
                holidaysSet.add(yyyy+"1005");
            }
            int hanGulDayChk = LocalDate.parse(yyyy+"1009", formatter).getDayOfWeek().getValue();
            if(hanGulDayChk == LD_SUNDAY) {      // 일요일
                holidaysSet.add(yyyy+"1010");
            }
            if(hanGulDayChk == LD_SATURDAY) {  // 토요일
